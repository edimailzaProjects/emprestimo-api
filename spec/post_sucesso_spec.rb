...
describe 'simulacao bem sucedida' do
    context 'quando uma nova simulacao for criada' do
        ...
        it { expect(result.parsed_response['cpf']). to eql '85093548018' }
    ...
    context 'quando o parametro seguro for falso' do
        let (:result) { ApiUser.save(build(:user_seguro_falso).to_hash) }
